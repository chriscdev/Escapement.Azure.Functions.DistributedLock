...
using Azure.Functions.DistributedLock.Middleware;
using Microsoft.Azure.Functions.Worker;
using Moq;

namespace Azure.Functions.DistributedLock.Test.Middleware
{
  public class DistributedLockMiddlewareResolveTemplateTests
  {
    private DistributedLockMiddleware CreateMiddleware()
    {
      var mockBlobService = new Mock<Storage.Blobs.BlobServiceClient>(MockBehavior.Strict);
      // Return a loose BlobContainerClient so constructor's CreateIfNotExists call is harmless
      mockBlobService.Setup(s => s.GetBlobContainerClient(It.IsAny<string>())).Returns(new Mock<Azure.Storage.Blobs.BlobContainerClient>().Object);
